...
class Node extends \MapasCulturais\Controller
{
    ...
    function GET_filters() {
        $filters = $this->plugin->config['filters'];
    ...
    function GET_verifyConnectionToken()
    {
        ...
        if ($token){
            ...
            if ($secret) {
                ...
                Plugin::log("token válido: {$token}");
                $app->halt(200, $secret);
            } else {
                Plugin::log("secret não encontrado: {$token}");
                $app->cache->delete("{$token}:verifier");
            }
        } else {
            Plugin::log("token inválido: {$token}");
        }
    }
    ...
    function GET_connect()
    {
        ...
        if ($connect_to && $create_token) {
            ...
            if ($create_secret) {
                ...
                Plugin::log("user app criado: {$user_app->name}");
        ...
        if ($id = $this->findEntityId($class_name, $network_id)) {
            ...
            foreach ($enabled_list as $local) {
                if ($this->compareEntityData($local, $data)) {
                    ...
                    $local->save(true);
                    Plugin::log("LINKED: {$local} => {$local->network__id}");
                    return;
                }
            }
        }
    ...
    function POST_createdFile()
    {
        ...
        $revisions = $this->postData['network__file_revisions'];
        ...
        if ($id = $this->findEntityId($owner_class, $owner_network_id)) {
            ...
            $owner->network__file_revisions = $owner->network__file_revisions ?: [];
            ...
            if($revision_id){
                $revisions = $owner->network__file_revisions;
                $revisions[] = $revision_id;
                $owner->network__file_revisions = $revisions;
            }
            // save the new entry's network ID (as placeholder)
            $network_ids = (array) $owner->network__file_ids;
            $network_ids[$network_id] = Plugin::UNKNOWN_ID;
            $owner->network__file_ids = $network_ids;
            ...
            $app->enqueueJob(Plugin::JOB_SLUG_DOWNLOADS, [
                ...
                "user" => $app->user->id,
                "owner" => $owner,
                "foreign_entity" => ['network__file_ids' => [$network_id => $data['id']]],
                "foreign_data" => [$data['group'] => $data]
            ]);
        }
        return;
    }
    ...
    function POST_createdMetaList()
    {
        ...
        $network_id = $this->postData["network__id"];
        $data = $this->plugin->unserializeEntity($this->postData["data"]);
        $revisions = $this->postData['network__metalist_revisions'];
        $revision_id = isset($revisions) ? end($revisions) : null;
        $classes = [
            Agent::class,
            Space::class,
        ...
        if (!in_array($owner_class, $classes)) {
            // @todo arrumar esse throw
            throw new PermissionDenied($app->user, $app->user, "create metalist");
        }
        // obtain the owner entity
        if ($id = $this->findEntityId($owner_class, $owner_network_id)) {
            $owner = $app->repo($owner_class)->find($id);
    ...
    function POST_deletedFile()
    {
        ...
        // obtain the owner entity
        if ($owner_id = $this->findEntityId($owner_class, $owner_network_id)) {
            $owner = $app->repo($owner_class)->find($owner_id);
            $owner->network__file_revisions = $owner->network__file_revisions ?: [];
            $owner->network__file_ids = $owner->network__file_ids ?: (object) [];
            if (in_array($revision_id, (array)$owner->network__file_revisions)) {
                $this->json("$network_id $revision_id already exists");
                return;
            }
            // add a revision
            if ($revision_id) {
                $revisions = (array)$owner->network__file_revisions;
                $revisions[] = $revision_id;
                $owner->network__file_revisions = $revisions;
            }
            // delete the item
            $ids = $owner->network__file_ids;
            $id = $ids->$network_id ?? null;
            if ($id && ($item = $app->repo("File")->find($id))) { 
                // stop revision ID from being created again
                $this->plugin->skip($item, [Plugin::SKIP_REVISION]);
    ...
    function POST_deletedMetaList()
    {
        ...
        if ($id = $this->findEntityId($owner_class, $owner_network_id)) {
            $owner = $app->repo($owner_class)->find($id);
            $owner->network__metalist_revisions = $owner->network__metalist_revisions ?: [];
            $owner->network__metalist_ids = $owner->network__metalist_ids ?: (object) [];
            if (in_array($revision_id, (array)$owner->network__metalist_revisions)) {
                $this->json("$network_id $revision_id already exists");
                return;
            }
            // add a revision
            ...
            $owner->network__metalist_revisions = $revisions;
            // delete the item
            $ids = $owner->network__metalist_ids;
            $id = $ids->$network_id ?? false;
            if ($id && ($item = $app->repo("MetaList")->find($id))) {
                // stop revision ID from being created again
                $this->plugin->skip($item, [Plugin::SKIP_REVISION]);
                // the owner must be saved since the IDs are kept there
                $item->delete(true);
    ...
    function POST_updatedEntity()
    {
        ...
        $network_id = $this->postData["network__id"];
        $data = $this->postData["data"];
        $revisions = $data["network__revisions"] ?? [];
        ...
        if ($id = $this->findEntityId($class_name, $network_id)) {
            ...
            if (in_array($revision_id, (array)$entity->network__revisions)) {
                $this->json("$network_id $revision_id already exists");
    ...
    function POST_updatedMetaList()
    {
        ...
        $network_id = $this->postData["network__id"];
        $data = $this->plugin->unserializeEntity($this->postData["data"]);
        $revision_key = "network__metalist_revisions";
        $network_ids_key = "network__metalist_ids";
        ...
        if ($id = $this->findentityId($owner_class, $owner_network_id)) {
            ...
            $owner->$network_ids_key = $owner->$network_ids_key ?: (object) [];
            ...
            // update the item
            $metalist_ids = $owner->$network_ids_key;
            $id = $metalist_ids->$network_id ?? null;
            if (!$id) {
            ...
            foreach ($input["remote"] as $foreign_data) {
                ...
                foreach ($input["local"] as $entity) {
                    if ($this->compareEntityData($entity, $foreign_data)) {
                        ...
                        Plugin::log("LINKED: {$entity} => {$entity->network__id}");                        
                        $linked = true;
                        break;
                    }
                }
                if (!$linked) {
                    ...
                    $entity = $this->plugin->createEntity($class, $foreign_data["network__id"], $foreign_data, $origin_node);
                    $data = $this->mergeEntity($entity, $foreign_data);
                }
            }
        }
        return;
    ...
    function POST_updateEntityNetworkId()
    {
        ...
        if ($id = $this->findEntityId($class_name, $current_network__id)) {
            ...
            $entity->save(true);
            Plugin::log("UPDATED: {$entity} => {$entity->network__id}");
        ...
        foreach ($foreign_data as $group => $group_data) {
            ...
            if (isset($group_data["id"])) {
                ...
                foreach ($group_data as $file) {
                    ...
                    if (count(array_filter(($entity_files[$group] ?? []), function ($item) use ($file) {
    ...
    {
        $app = App::i();
        $list_groups = array_keys($app->getRegisteredMetaListGroupsByEntity($entity));
        foreach ($foreign_data as $group => $group_data) {
            ...
            foreach ($group_data as $list_item) {
                ...
                $network_ids = (array) $entity->network__metalist_ids;
                $network_ids[$network_id] = $new_item->id;
                $entity->network__metalist_ids = $network_ids;
            }
    ...
    function compareAgentData(Agent $agent, array $foreign_data) {
        ...
        $anetwork__id = $agent->network__id ?: ''; // em metadados não pode usar o operador ??
        ...
        $adoc = preg_replace('#[^0-9]#', '', $agent->documento ?: ''); // em metadados não pode usar o operador ??
        ...
        if ($data->nomeCompleto ?? null) {
            ...
            $aname = Utils::slugify($agent->nomeCompleto ?: ''); // em metadados não pode usar o operador ??
        ...
        if ($data->name ?? null) {
            ...
            $aname = Utils::slugify($agent->name ?: ''); // em metadados não pode usar o operador ??
    ...
    function compareSpaceData(Space $space, array $foreign_data) {
        ...
        $anetwork__id = $space->network__id ?: ''; // em metadados não pode usar o operador ??
        $fnetwork__id = $data->network__id ?? '';
        if ($anetwork__id && $anetwork__id == $fnetwork__id) {
            return true;
        }
        ...
        $adoc = preg_replace('#[^0-9]#', '', $space->cnpj ?: ''); // em metadados não pode usar o operador ??
        ...
        if ($data->name ?? null) {
            $fname = Utils::slugify($data->name ?? '');
            $aname = Utils::slugify($space->name ?: ''); // em metadados não pode usar o operador ??
    ...
    function mergeEntity(Entity $entity, array $foreign_data)
    {
        ...
        $skip_list = [
            ...
            "network__id", "network__revisions",
        ...
        if ($use_files) {
            $this->bootstrapFiles($entity, $use_files, $foreign_data, $origin_node);
        }

        if ($use_metalist) {
            $this->bootstrapMetaLists($entity, $use_metalist, $foreign_data);
        }
        return $data;
    }
